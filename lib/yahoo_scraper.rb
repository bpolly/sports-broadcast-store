class YahooScraper
  ...
  def scrape_nba
    team = Team.find_given_nickname('cavs').first

    SEASON_MONTHS.each do |month|
      cavs_schedule = "http://scores.nbcsports.msnbc.com/nba/teamstats.asp?teamno=5&type=schedule&month=#{month}"
      month = month.to_i
      doc = Nokogiri::HTML(open(cavs_schedule))
      ...
      dates = doc.css(".shsTable tr").each do |row|
        ...
        if(cols.count > 2 && (row.attributes['class'].value != 'shsTableTtlRow') && (row.attributes['class'].value != 'shsColTtlRow'))
          ...
          time = time_extractor.match(cols[2].text)[1]
          tv_networks = cols[3].text.gsub(/[[:space:]]/, '').split('/').join(',')
          ...
          game = Game.new(home_team_id: home_team_id, away_team_id: away_team_id, date: date_time, tv_networks: tv_networks, league: 'nba')
          game.save
        end
      end
    end
